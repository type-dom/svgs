import { TypeProps, SvgPath, TypeSvgSvg } from '@type-dom/framework';
export class FlArrowExitFilledSvg extends TypeSvgSvg {
  className: 'FlArrowExitFilledSvg';
  override childNodes: SvgPath[];
  constructor(params: TypeProps = {}) {
    super();
    this.className = 'FlArrowExitFilledSvg';
    this.attr.addObj({
      name: 'FlArrowExitFilledSvg',
      title: 'FlArrowExitFilledSvg'
    });
    this.attr.addObj({
      viewBox: '0 0 20 20',
    });
    this.resetSize(24, 24);
    this.childNodes = [];
    const path0 = new SvgPath({ attrObj: { fill: 'currentColor' }});
    path0.setData(
      'M12.75 17.5C13.1642 17.5 13.5 17.1642 13.5 16.75C13.5 16.3358 13.1642 16 12.75 16H6.5C5.39543 16 4.5 15.1046 4.5 14V6C4.5 4.89543 5.39543 4 6.5 4H12.75C13.1642 4 13.5 3.66421 13.5 3.25C13.5 2.83579 13.1642 2.5 12.75 2.5H6.5C4.567 2.5 3 4.067 3 6V14C3 15.933 4.567 17.5 6.5 17.5H12.75ZM13.7413 6.1989C14.0457 5.91795 14.5201 5.93693 14.8011 6.24129L17.8011 9.49129C18.0663 9.77859 18.0663 10.2214 17.8011 10.5087L14.8011 13.7587C14.5201 14.0631 14.0457 14.0821 13.7413 13.8011C13.4369 13.5202 13.4179 13.0457 13.6989 12.7413L15.537 10.75H7.75C7.33579 10.75 7 10.4142 7 10C7 9.58579 7.33579 9.25 7.75 9.25H15.537L13.6989 7.25871C13.4179 6.95435 13.4369 6.47985 13.7413 6.1989Z',
    );
    this.addChild(path0);
    this.useParams(params);
  }
}
