import { ITypeConfig, SvgPath, TypeSvgSvg } from '@type-dom/framework';
export class FlArchiveArrowBackFilledSvg extends TypeSvgSvg {
  className: 'FlArchiveArrowBackFilledSvg';
  override childNodes: SvgPath[];
  constructor(params: ITypeConfig = {}) {
    super();
    this.className = 'FlArchiveArrowBackFilledSvg';
    this.attr.addObj({
      name: 'FlArchiveArrowBackFilledSvg',
      title: 'FlArchiveArrowBackFilledSvg'
    });
    this.attr.addObj({
      viewBox: '0 0 20 20',
    });
    this.resetSize(24, 24);
    this.childNodes = [];
    const path0 = new SvgPath({ attrObj: { fill: 'currentColor' }});
    path0.setData(
      'M2 4.25C2 3.55964 2.55964 3 3.25 3H16.75C17.4404 3 18 3.55964 18 4.25V5.75C18 6.44036 17.4404 7 16.75 7H3.25C2.55964 7 2 6.44036 2 5.75V4.25ZM3 8H17V9.59971C16.2499 9.21628 15.4002 9 14.5 9C13.3226 9 12.2316 9.36997 11.3369 10H8.5C8.22386 10 8 10.2239 8 10.5C8 10.7761 8.22386 11 8.5 11H10.2572C9.47182 11.9509 9 13.1704 9 14.5C9 15.4002 9.21628 16.2499 9.59971 17H6C4.34315 17 3 15.6569 3 14V8ZM14.5 19C16.9853 19 19 16.9853 19 14.5C19 12.0147 16.9853 10 14.5 10C12.0147 10 10 12.0147 10 14.5C10 16.9853 12.0147 19 14.5 19ZM13.6036 12.6036L12.7071 13.5H14.25C15.7688 13.5 17 14.7312 17 16.25V16.5C17 16.7761 16.7761 17 16.5 17C16.2239 17 16 16.7761 16 16.5V16.25C16 15.2835 15.2165 14.5 14.25 14.5H12.7071L13.6036 15.3964C13.7988 15.5917 13.7988 15.9083 13.6036 16.1036C13.4083 16.2988 13.0917 16.2988 12.8964 16.1036L11.1443 14.3514C11.0974 14.304 11.062 14.2495 11.0379 14.1914C11.0135 14.1324 11 14.0678 11 14C11 13.9322 11.0135 13.8676 11.0379 13.8086C11.0623 13.7496 11.0985 13.6944 11.1464 13.6464L12.8964 11.8964C13.0917 11.7012 13.4083 11.7012 13.6036 11.8964C13.7988 12.0917 13.7988 12.4083 13.6036 12.6036Z',
    );
    this.addChild(path0);
    this.useParams(params);
  }
}
