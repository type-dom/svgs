import { TypeProps, SvgPath, TypeSvgSvg } from '@type-dom/framework';
export class FlSlideMultipleArrowRightFilledSvg extends TypeSvgSvg {
  className: 'FlSlideMultipleArrowRightFilledSvg';
  override childNodes: SvgPath[];
  constructor(params: TypeProps = {}) {
    super();
    this.className = 'FlSlideMultipleArrowRightFilledSvg';
    this.attr.addObj({
      name: 'FlSlideMultipleArrowRightFilledSvg',
      title: 'FlSlideMultipleArrowRightFilledSvg'
    });
    this.attr.addObj({
      viewBox: '0 0 20 20',
    });
    this.resetSize(24, 24);
    this.childNodes = [];
    const path0 = new SvgPath({ attrObj: { fill: 'currentColor' }});
    path0.setData(
      'M6 3C3.79086 3 2 4.79086 2 7V12C2 13.1046 2.89543 14 4 14L4 8C4 6.34315 5.34315 5 7 5H15C15 3.89543 14.1046 3 13 3H6ZM7 6C5.89543 6 5 6.89543 5 8V15C5 16.1046 5.89543 17 7 17H9.59971C9.21628 16.2499 9 15.4002 9 14.5C9 11.4624 11.4624 9 14.5 9C15.8296 9 17.0491 9.47182 18 10.2572V8C18 6.89543 17.1046 6 16 6H7ZM14.5 10C16.9853 10 19 12.0147 19 14.5C19 16.9853 16.9853 19 14.5 19C12.0147 19 10 16.9853 10 14.5C10 12.0147 12.0147 10 14.5 10ZM16.8532 14.854L16.8557 14.8514C16.9026 14.804 16.938 14.7495 16.9621 14.6914C16.9861 14.6333 16.9996 14.5697 17 14.503L17 14.5L17 14.497C16.9996 14.4303 16.9861 14.3667 16.9621 14.3086C16.9377 14.2496 16.9015 14.1944 16.8536 14.1464L14.8536 12.1464C14.6583 11.9512 14.3417 11.9512 14.1464 12.1464C13.9512 12.3417 13.9512 12.6583 14.1464 12.8536L15.2929 14H12.5C12.2239 14 12 14.2239 12 14.5C12 14.7761 12.2239 15 12.5 15H15.2929L14.1464 16.1464C13.9512 16.3417 13.9512 16.6583 14.1464 16.8536C14.3417 17.0488 14.6583 17.0488 14.8536 16.8536L16.8532 14.854Z',
    );
    this.addChild(path0);
    this.useParams(params);
  }
}
