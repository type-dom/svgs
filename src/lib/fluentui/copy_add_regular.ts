import { TypeProps, SvgPath, TypeSvgSvg } from '@type-dom/framework';
export class FlCopyAddRegularSvg extends TypeSvgSvg {
  className: 'FlCopyAddRegularSvg';
  override childNodes: SvgPath[];
  constructor(params: TypeProps = {}) {
    super();
    this.className = 'FlCopyAddRegularSvg';
    this.attr.addObj({
      name: 'FlCopyAddRegularSvg',
      title: 'FlCopyAddRegularSvg'
    });
    this.attr.addObj({
      viewBox: '0 0 20 20',
    });
    this.resetSize(24, 24);
    this.childNodes = [];
    const path0 = new SvgPath({ attrObj: { fill: 'currentColor' }});
    path0.setData(
      'M8 2C6.89543 2 6 2.89543 6 4V14C6 15.1046 6.89543 16 8 16H9.20703C9.11588 15.6777 9.05337 15.3434 9.02242 15H8C7.44772 15 7 14.5523 7 14V4C7 3.44772 7.44772 3 8 3H14C14.5523 3 15 3.44772 15 4V9.02242C15.3434 9.05337 15.6777 9.11588 16 9.20703V4C16 2.89543 15.1046 2 14 2H8ZM7.5 17H9.59972C9.78261 17.3578 10.0035 17.6929 10.2572 18H7.5C5.567 18 4 16.433 4 14.5V6.00001C4 5.25973 4.4022 4.61339 5 4.26758V14.5C5 15.8807 6.11929 17 7.5 17ZM19 14.5C19 16.9853 16.9853 19 14.5 19C12.0147 19 10 16.9853 10 14.5C10 12.0147 12.0147 10 14.5 10C16.9853 10 19 12.0147 19 14.5ZM15 12.5C15 12.2239 14.7761 12 14.5 12C14.2239 12 14 12.2239 14 12.5V14H12.5C12.2239 14 12 14.2239 12 14.5C12 14.7761 12.2239 15 12.5 15H14V16.5C14 16.7761 14.2239 17 14.5 17C14.7761 17 15 16.7761 15 16.5V15H16.5C16.7761 15 17 14.7761 17 14.5C17 14.2239 16.7761 14 16.5 14H15V12.5Z',
    );
    this.addChild(path0);
    this.useParams(params);
  }
}
