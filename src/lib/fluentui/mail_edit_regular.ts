import { ITypeConfig, SvgPath, TypeSvgSvg } from '@type-dom/framework';
export class FlMailEditRegularSvg extends TypeSvgSvg {
  className: 'FlMailEditRegularSvg';
  override childNodes: SvgPath[];
  constructor(params: ITypeConfig = {}) {
    super();
    this.className = 'FlMailEditRegularSvg';
    this.attr.addObj({
      name: 'FlMailEditRegularSvg',
      title: 'FlMailEditRegularSvg'
    });
    this.attr.addObj({
      viewBox: '0 0 20 20',
    });
    this.resetSize(24, 24);
    this.childNodes = [];
    const path0 = new SvgPath({ attrObj: { fill: 'currentColor' }});
    path0.setData(
      'M15.5 3.00098C16.8807 3.00098 18 4.12026 18 5.50098V9.13392C17.6757 9.03129 17.3368 8.98764 17 9.003V6.96198L10.2535 10.9319C10.1231 11.0086 9.96661 11.0214 9.82751 10.9703L9.74649 10.9319L3 6.96398V13.501C3 14.3294 3.67157 15.001 4.5 15.001H9.98428C9.7571 15.301 9.58423 15.6395 9.47436 16.001H4.5C3.11929 16.001 2 14.8817 2 13.501V5.50098C2 4.12026 3.11929 3.00098 4.5 3.00098H15.5ZM15.5 4.00098H4.5C3.67157 4.00098 3 4.67255 3 5.50098V5.80298L10 9.92089L17 5.80198V5.50098C17 4.67255 16.3284 4.00098 15.5 4.00098ZM10.9798 15.3772L15.8092 10.5478C16.5395 9.81741 17.7237 9.81741 18.454 10.5478C19.1843 11.2781 19.1843 12.4622 18.454 13.1926L13.6246 18.022C13.343 18.3036 12.9902 18.5033 12.6039 18.5999L11.106 18.9744C10.4546 19.1372 9.86451 18.5472 10.0274 17.8958L10.4018 16.3979C10.4984 16.0116 10.6982 15.6588 10.9798 15.3772Z',
    );
    this.addChild(path0);
    this.useParams(params);
  }
}
