import { TypeProps, SvgPath, TypeSvgSvg } from '@type-dom/framework';
export class FlPhoneArrowRightRegularSvg extends TypeSvgSvg {
  className: 'FlPhoneArrowRightRegularSvg';
  override childNodes: SvgPath[];
  constructor(params: TypeProps = {}) {
    super();
    this.className = 'FlPhoneArrowRightRegularSvg';
    this.attr.addObj({
      name: 'FlPhoneArrowRightRegularSvg',
      title: 'FlPhoneArrowRightRegularSvg'
    });
    this.attr.addObj({
      viewBox: '0 0 20 20',
    });
    this.resetSize(24, 24);
    this.childNodes = [];
    const path0 = new SvgPath({ attrObj: { fill: 'currentColor' }});
    path0.setData(
      'M5.5 2C4.67157 2 4 2.67157 4 3.5V16.5C4 17.3284 4.67157 18 5.5 18H10.2572C10.0035 17.6929 9.78261 17.3578 9.59971 17H5.5C5.22386 17 5 16.7761 5 16.5V3.5C5 3.22386 5.22386 3 5.5 3H12.5C12.7761 3 13 3.22386 13 3.5V9.20703C13.3223 9.11588 13.6566 9.05337 14 9.02242V3.5C14 2.67157 13.3284 2 12.5 2H5.5ZM9 14.5C9 14.3315 9.00758 14.1647 9.02242 14H7.5C7.22386 14 7 14.2239 7 14.5C7 14.7761 7.22386 15 7.5 15H9.02242C9.00758 14.8353 9 14.6685 9 14.5ZM14.5 10C16.9853 10 19 12.0147 19 14.5C19 16.9853 16.9853 19 14.5 19C12.0147 19 10 16.9853 10 14.5C10 12.0147 12.0147 10 14.5 10ZM16.8532 14.854L16.8557 14.8514C16.9026 14.804 16.938 14.7495 16.9621 14.6914C16.9861 14.6333 16.9996 14.5697 17 14.503L17 14.5L17 14.497C16.9996 14.4303 16.9861 14.3667 16.9621 14.3086C16.9377 14.2496 16.9015 14.1944 16.8536 14.1464L14.8536 12.1464C14.6583 11.9512 14.3417 11.9512 14.1464 12.1464C13.9512 12.3417 13.9512 12.6583 14.1464 12.8536L15.2929 14H12.5C12.2239 14 12 14.2239 12 14.5C12 14.7761 12.2239 15 12.5 15H15.2929L14.1464 16.1464C13.9512 16.3417 13.9512 16.6583 14.1464 16.8536C14.3417 17.0488 14.6583 17.0488 14.8536 16.8536L16.8532 14.854Z',
    );
    this.addChild(path0);
    this.useParams(params);
  }
}
