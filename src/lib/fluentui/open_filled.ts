import { ITypeConfig, SvgPath, TypeSvgSvg } from '@type-dom/framework';
export class FlOpenFilledSvg extends TypeSvgSvg {
  className: 'FlOpenFilledSvg';
  override childNodes: SvgPath[];
  constructor(params: ITypeConfig = {}) {
    super();
    this.className = 'FlOpenFilledSvg';
    this.attr.addObj({
      name: 'FlOpenFilledSvg',
      title: 'FlOpenFilledSvg'
    });
    this.attr.addObj({
      viewBox: '0 0 20 20',
    });
    this.resetSize(24, 24);
    this.childNodes = [];
    const path0 = new SvgPath({ attrObj: { fill: 'currentColor' }});
    path0.setData(
      'M6.25 4.5C5.2835 4.5 4.5 5.2835 4.5 6.25V13.75C4.5 14.7165 5.2835 15.5 6.25 15.5H13.75C14.7165 15.5 15.5 14.7165 15.5 13.75V11.75C15.5 11.3358 15.8358 11 16.25 11C16.6642 11 17 11.3358 17 11.75V13.75C17 15.5449 15.5449 17 13.75 17H6.25C4.45507 17 3 15.5449 3 13.75V6.25C3 4.45507 4.45507 3 6.25 3H8.25C8.66421 3 9 3.33579 9 3.75C9 4.16421 8.66421 4.5 8.25 4.5H6.25ZM10.5 3.75C10.5 3.33579 10.8358 3 11.25 3H16.25C16.6642 3 17 3.33579 17 3.75V8.75C17 9.16421 16.6642 9.5 16.25 9.5C15.8358 9.5 15.5 9.16421 15.5 8.75V5.56066L11.7803 9.28033C11.4874 9.57322 11.0126 9.57322 10.7197 9.28033C10.4268 8.98744 10.4268 8.51256 10.7197 8.21967L14.4393 4.5H11.25C10.8358 4.5 10.5 4.16421 10.5 3.75Z',
    );
    this.addChild(path0);
    this.useParams(params);
  }
}
