import { TypeProps, SvgPath, TypeSvgSvg } from '@type-dom/framework';
export class FlTextIndentDecreaseRtlRegularSvg extends TypeSvgSvg {
  className: 'FlTextIndentDecreaseRtlRegularSvg';
  override childNodes: SvgPath[];
  constructor(params: TypeProps = {}) {
    super();
    this.className = 'FlTextIndentDecreaseRtlRegularSvg';
    this.attr.addObj({
      name: 'FlTextIndentDecreaseRtlRegularSvg',
      title: 'FlTextIndentDecreaseRtlRegularSvg'
    });
    this.attr.addObj({
      viewBox: '0 0 20 20',
    });
    this.resetSize(24, 24);
    this.childNodes = [];
    const path0 = new SvgPath({ attrObj: { fill: 'currentColor' }});
    path0.setData(
      'M5.5 4C5.22386 4 5 4.22386 5 4.5C5 4.77614 5.22386 5 5.5 5H13.5C13.7761 5 14 4.77614 14 4.5C14 4.22386 13.7761 4 13.5 4H5.5ZM2.5 9C2.22386 9 2 9.22386 2 9.5C2 9.77614 2.22386 10 2.5 10H13.5C13.7761 10 14 9.77614 14 9.5C14 9.22386 13.7761 9 13.5 9H2.5ZM7 14.5C7 14.2239 7.22386 14 7.5 14H13.5C13.7761 14 14 14.2239 14 14.5C14 14.7761 13.7761 15 13.5 15H7.5C7.22386 15 7 14.7761 7 14.5ZM16.4532 7.73647C16.2579 7.54121 15.9413 7.54121 15.7461 7.73647C15.5508 7.93174 15.5508 8.24832 15.7461 8.44358L16.8067 9.50424L15.7461 10.5649C15.5508 10.7602 15.5508 11.0767 15.7461 11.272C15.9413 11.4673 16.2579 11.4673 16.4532 11.272L17.8674 9.85779C18.0626 9.66253 18.0626 9.34595 17.8674 9.15069L16.4532 7.73647Z',
    );
    this.addChild(path0);
    this.useParams(params);
  }
}
