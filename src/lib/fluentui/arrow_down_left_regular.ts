import { ITypeConfig, SvgPath, TypeSvgSvg } from '@type-dom/framework';
export class FlArrowDownLeftRegularSvg extends TypeSvgSvg {
  className: 'FlArrowDownLeftRegularSvg';
  override childNodes: SvgPath[];
  constructor(params: ITypeConfig = {}) {
    super();
    this.className = 'FlArrowDownLeftRegularSvg';
    this.attr.addObj({
      name: 'FlArrowDownLeftRegularSvg',
      title: 'FlArrowDownLeftRegularSvg'
    });
    this.attr.addObj({
      viewBox: '0 0 20 20',
    });
    this.resetSize(24, 24);
    this.childNodes = [];
    const path0 = new SvgPath({ attrObj: { fill: 'currentColor' }});
    path0.setData(
      'M11.5 16C11.7761 16 12 16.2239 12 16.5C12 16.7761 11.7761 17 11.5 17H3.5C3.22386 17 3 16.7761 3 16.5V8.5C3 8.22386 3.22386 8 3.5 8C3.77614 8 3.99999 8.22386 3.99999 8.5V15.2929L16.1465 3.14645C16.3417 2.95118 16.6583 2.95119 16.8536 3.14645C17.0488 3.34171 17.0488 3.65829 16.8536 3.85355L4.7071 16H11.5Z',
    );
    this.addChild(path0);
    this.useParams(params);
  }
}
