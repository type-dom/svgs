import { TypeProps, SvgPath, TypeSvgSvg } from '@type-dom/framework';
export class FlPuzzlePieceRegularSvg extends TypeSvgSvg {
  className: 'FlPuzzlePieceRegularSvg';
  override childNodes: SvgPath[];
  constructor(params: TypeProps = {}) {
    super();
    this.className = 'FlPuzzlePieceRegularSvg';
    this.attr.addObj({
      name: 'FlPuzzlePieceRegularSvg',
      title: 'FlPuzzlePieceRegularSvg'
    });
    this.attr.addObj({
      viewBox: '0 0 20 20',
    });
    this.resetSize(24, 24);
    this.childNodes = [];
    const path0 = new SvgPath({ attrObj: { fill: 'currentColor' }});
    path0.setData(
      'M11 3C10.3096 3 9.75 3.55964 9.75 4.25V5H6.5C6.22386 5 6 5.22386 6 5.5V8.75H5.25C4.55964 8.75 4 9.30964 4 10C4 10.6904 4.55964 11.25 5.25 11.25H6V14.5C6 14.7761 6.22386 15 6.5 15H9.75V15.75C9.75 16.4404 10.3096 17 11 17C11.6904 17 12.25 16.4404 12.25 15.75V15H15.5C15.7761 15 16 14.7761 16 14.5V12.25H15.25C14.0074 12.25 13 11.2426 13 10C13 8.75736 14.0074 7.75 15.25 7.75H16V5.5C16 5.22386 15.7761 5 15.5 5H12.25V4.25C12.25 3.55964 11.6904 3 11 3ZM8.76373 4C8.88809 2.87501 9.84186 2 11 2C12.1581 2 13.1119 2.87501 13.2363 4H15.5C16.3284 4 17 4.67157 17 5.5V8.75H15.25C14.5596 8.75 14 9.30964 14 10C14 10.6904 14.5596 11.25 15.25 11.25H17V14.5C17 15.3284 16.3284 16 15.5 16H13.2363C13.1119 17.125 12.1581 18 11 18C9.84186 18 8.88809 17.125 8.76373 16H6.5C5.67157 16 5 15.3284 5 14.5V12.2363C3.87501 12.1119 3 11.1581 3 10C3 8.84186 3.87501 7.88809 5 7.76373V5.5C5 4.67157 5.67157 4 6.5 4H8.76373Z',
    );
    this.addChild(path0);
    this.useParams(params);
  }
}
