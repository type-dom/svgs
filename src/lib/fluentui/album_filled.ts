import { ITypeConfig, SvgPath, TypeSvgSvg } from '@type-dom/framework';
export class FlAlbumFilledSvg extends TypeSvgSvg {
  className: 'FlAlbumFilledSvg';
  override childNodes: SvgPath[];
  constructor(params: ITypeConfig = {}) {
    super();
    this.className = 'FlAlbumFilledSvg';
    this.attr.addObj({
      name: 'FlAlbumFilledSvg',
      title: 'FlAlbumFilledSvg'
    });
    this.attr.addObj({
      viewBox: '0 0 20 20',
    });
    this.resetSize(24, 24);
    this.childNodes = [];
    const path0 = new SvgPath({ attrObj: { fill: 'currentColor' }});
    path0.setData(
      'M2 6C2 4.89543 2.89543 4 4 4V16C2.89543 16 2 15.1046 2 14V6ZM9 8.5C9 8.22386 9.22386 8 9.5 8H13.5C13.7761 8 14 8.22386 14 8.5V9.5C14 9.77614 13.7761 10 13.5 10H9.5C9.22386 10 9 9.77614 9 9.5V8.5ZM5 16V4H16C17.1046 4 18 4.89543 18 6V14C18 15.1046 17.1046 16 16 16H5ZM9.5 7C8.67157 7 8 7.67157 8 8.5V9.5C8 10.3284 8.67157 11 9.5 11H13.5C14.3284 11 15 10.3284 15 9.5V8.5C15 7.67157 14.3284 7 13.5 7H9.5Z',
    );
    this.addChild(path0);
    this.useParams(params);
  }
}
