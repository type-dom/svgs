import { ITypeConfig, SvgPath, TypeSvgSvg } from '@type-dom/framework';
export class FlAlignSpaceEvenlyHorizontalFilledSvg extends TypeSvgSvg {
  className: 'FlAlignSpaceEvenlyHorizontalFilledSvg';
  override childNodes: SvgPath[];
  constructor(params: ITypeConfig = {}) {
    super();
    this.className = 'FlAlignSpaceEvenlyHorizontalFilledSvg';
    this.attr.addObj({
      name: 'FlAlignSpaceEvenlyHorizontalFilledSvg',
      title: 'FlAlignSpaceEvenlyHorizontalFilledSvg'
    });
    this.attr.addObj({
      viewBox: '0 0 20 20',
    });
    this.resetSize(24, 24);
    this.childNodes = [];
    const path0 = new SvgPath({ attrObj: { fill: 'currentColor' }});
    path0.setData(
      'M4 16C5.10457 16 6 15.1046 6 14V6C6 4.89543 5.10457 4 4 4H3C1.89543 4 1 4.89543 1 6V14C1 15.1046 1.89543 16 3 16H4Z',
    );
    this.addChild(path0);
    const path1 = new SvgPath({ attrObj: { fill: 'currentColor' }});
    path1.setData(
      'M10 16C11.1046 16 12 15.1046 12 14V6C12 4.89543 11.1046 4 10 4H9C7.89543 4 7 4.89543 7 6V14C7 15.1046 7.89543 16 9 16H10Z',
    );
    this.addChild(path1);
    const path2 = new SvgPath({ attrObj: { fill: 'currentColor' }});
    path2.setData(
      'M16 16C17.1046 16 18 15.1046 18 14V6C18 4.89543 17.1046 4 16 4H15C13.8954 4 13 4.89543 13 6V14C13 15.1046 13.8954 16 15 16H16Z',
    );
    this.addChild(path2);
    this.useParams(params);
  }
}
