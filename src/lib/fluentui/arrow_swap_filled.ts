import { TypeProps, SvgPath, TypeSvgSvg } from '@type-dom/framework';
export class FlArrowSwapFilledSvg extends TypeSvgSvg {
  className: 'FlArrowSwapFilledSvg';
  override childNodes: SvgPath[];
  constructor(params: TypeProps = {}) {
    super();
    this.className = 'FlArrowSwapFilledSvg';
    this.attr.addObj({
      name: 'FlArrowSwapFilledSvg',
      title: 'FlArrowSwapFilledSvg'
    });
    this.attr.addObj({
      viewBox: '0 0 20 20',
    });
    this.resetSize(24, 24);
    this.childNodes = [];
    const path0 = new SvgPath({ attrObj: { fill: 'currentColor' }});
    path0.setData(
      'M12.7803 3.21967C12.4874 2.92678 12.0126 2.92678 11.7197 3.21967C11.4268 3.51256 11.4268 3.98744 11.7197 4.28033L13.4393 6H4.75C4.33579 6 4 6.33579 4 6.75C4 7.16421 4.33579 7.5 4.75 7.5H13.4393L11.7197 9.21967C11.4268 9.51256 11.4268 9.98744 11.7197 10.2803C12.0126 10.5732 12.4874 10.5732 12.7803 10.2803L15.7803 7.28033C16.0732 6.98744 16.0732 6.51256 15.7803 6.21967L12.7803 3.21967ZM8.28033 10.7803C8.57322 10.4874 8.57322 10.0126 8.28033 9.71967C7.98744 9.42678 7.51256 9.42678 7.21967 9.71967L4.21967 12.7197C4.07902 12.8603 4 13.0511 4 13.25C4 13.4489 4.07902 13.6397 4.21967 13.7803L7.21967 16.7803C7.51256 17.0732 7.98744 17.0732 8.28033 16.7803C8.57322 16.4874 8.57322 16.0126 8.28033 15.7197L6.56066 14H15.25C15.6642 14 16 13.6642 16 13.25C16 12.8358 15.6642 12.5 15.25 12.5H6.56066L8.28033 10.7803Z',
    );
    this.addChild(path0);
    this.useParams(params);
  }
}
