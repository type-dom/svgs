import { TypeProps, SvgPath, TypeSvgSvg } from '@type-dom/framework';
export class FlArrowEnterUpFilledSvg extends TypeSvgSvg {
  className: 'FlArrowEnterUpFilledSvg';
  override childNodes: SvgPath[];
  constructor(params: TypeProps = {}) {
    super();
    this.className = 'FlArrowEnterUpFilledSvg';
    this.attr.addObj({
      name: 'FlArrowEnterUpFilledSvg',
      title: 'FlArrowEnterUpFilledSvg'
    });
    this.attr.addObj({
      viewBox: '0 0 20 20',
    });
    this.resetSize(24, 24);
    this.childNodes = [];
    const path0 = new SvgPath({ attrObj: { fill: 'currentColor' }});
    path0.setData(
      'M8.79615 2.23598C8.65442 2.08539 8.4568 2 8.25 2C8.0432 2 7.84558 2.08539 7.70385 2.23598L3.70385 6.48598C3.41996 6.78761 3.43435 7.26226 3.73598 7.54615C4.03761 7.83004 4.51226 7.81565 4.79615 7.51402L7.5 4.64119V14.75C7.5 16.5449 8.95507 18 10.75 18H15.25C15.6642 18 16 17.6642 16 17.25C16 16.8358 15.6642 16.5 15.25 16.5H10.75C9.7835 16.5 9 15.7165 9 14.75V4.64118L11.7039 7.51402C11.9877 7.81565 12.4624 7.83004 12.764 7.54615C13.0657 7.26226 13.08 6.78761 12.7962 6.48598L8.79615 2.23598Z',
    );
    this.addChild(path0);
    this.useParams(params);
  }
}
