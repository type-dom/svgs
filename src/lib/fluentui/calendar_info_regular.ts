import { ITypeConfig, SvgPath, TypeSvgSvg } from '@type-dom/framework';
export class FlCalendarInfoRegularSvg extends TypeSvgSvg {
  className: 'FlCalendarInfoRegularSvg';
  override childNodes: SvgPath[];
  constructor(params: ITypeConfig = {}) {
    super();
    this.className = 'FlCalendarInfoRegularSvg';
    this.attr.addObj({
      name: 'FlCalendarInfoRegularSvg',
      title: 'FlCalendarInfoRegularSvg'
    });
    this.attr.addObj({
      viewBox: '0 0 20 20',
    });
    this.resetSize(24, 24);
    this.childNodes = [];
    const path0 = new SvgPath({ attrObj: { fill: 'currentColor' }});
    path0.setData(
      'M14.5 3C15.8807 3 17 4.11929 17 5.5V9.59971C16.6832 9.43777 16.3486 9.30564 16 9.20703V7H4V14.5C4 15.3284 4.67157 16 5.5 16H9.20703C9.30564 16.3486 9.43777 16.6832 9.59971 17H5.5C4.11929 17 3 15.8807 3 14.5V5.5C3 4.11929 4.11929 3 5.5 3H14.5ZM14.5 4H5.5C4.67157 4 4 4.67157 4 5.5V6H16V5.5C16 4.67157 15.3284 4 14.5 4ZM13.875 12.5C13.875 12.1548 14.1548 11.875 14.5 11.875C14.8452 11.875 15.125 12.1548 15.125 12.5C15.125 12.8452 14.8452 13.125 14.5 13.125C14.1548 13.125 13.875 12.8452 13.875 12.5ZM15 16.5C15 16.7761 14.7761 17 14.5 17C14.2239 17 14 16.7761 14 16.5V14.5C14 14.2239 14.2239 14 14.5 14C14.7761 14 15 14.2239 15 14.5V16.5ZM10 14.5C10 12.0147 12.0147 10 14.5 10C16.9853 10 19 12.0147 19 14.5C19 16.9853 16.9853 19 14.5 19C12.0147 19 10 16.9853 10 14.5ZM11 14.5C11 16.433 12.567 18 14.5 18C16.433 18 18 16.433 18 14.5C18 12.567 16.433 11 14.5 11C12.567 11 11 12.567 11 14.5Z',
    );
    this.addChild(path0);
    this.useParams(params);
  }
}
