import { TypeProps, SvgPath, TypeSvgSvg } from '@type-dom/framework';
export class FlNoteAddRegularSvg extends TypeSvgSvg {
  className: 'FlNoteAddRegularSvg';
  override childNodes: SvgPath[];
  constructor(params: TypeProps = {}) {
    super();
    this.className = 'FlNoteAddRegularSvg';
    this.attr.addObj({
      name: 'FlNoteAddRegularSvg',
      title: 'FlNoteAddRegularSvg'
    });
    this.attr.addObj({
      viewBox: '0 0 20 20',
    });
    this.resetSize(24, 24);
    this.childNodes = [];
    const path0 = new SvgPath({ attrObj: { fill: 'currentColor' }});
    path0.setData(
      'M10 5.5C10 7.98528 7.98528 10 5.5 10C3.01472 10 1 7.98528 1 5.5C1 3.01472 3.01472 1 5.5 1C7.98528 1 10 3.01472 10 5.5ZM6 3.5C6 3.22386 5.77614 3 5.5 3C5.22386 3 5 3.22386 5 3.5V5H3.5C3.22386 5 3 5.22386 3 5.5C3 5.77614 3.22386 6 3.5 6H5L5 7.5C5 7.77614 5.22386 8 5.5 8C5.77614 8 6 7.77614 6 7.5V6H7.5C7.77614 6 8 5.77614 8 5.5C8 5.22386 7.77614 5 7.5 5H6V3.5ZM14 4H10.793C10.6944 3.65136 10.5622 3.31679 10.4003 3H14C15.5977 3 16.9037 4.24892 16.9949 5.82373L17 6V10.3787C17 10.8502 16.8335 11.3045 16.5331 11.6631L16.4142 11.7929L11.7929 16.4142C11.4595 16.7476 11.0205 16.9511 10.5545 16.9923L10.3787 17H6C4.40232 17 3.09634 15.7511 3.00509 14.1763L3 14V10.4003C3.31679 10.5622 3.65136 10.6944 4 10.793V14C4 15.0544 4.81588 15.9182 5.85074 15.9945L6 16H10V13C10 11.4023 11.2489 10.0963 12.8237 10.0051L13 10H16V6C16 4.94564 15.1841 4.08183 14.1493 4.00549L14 4ZM15.7825 11.0013L13 11C11.9456 11 11.0818 11.8159 11.0055 12.8507L11 13V15.781L11.0858 15.7071L15.7071 11.0858C15.7339 11.059 15.7591 11.0307 15.7825 11.0013Z',
    );
    this.addChild(path0);
    this.useParams(params);
  }
}
