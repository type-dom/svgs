import { TypeProps, SvgPath, TypeSvgSvg } from '@type-dom/framework';
export class FlSaveEditRegularSvg extends TypeSvgSvg {
  className: 'FlSaveEditRegularSvg';
  override childNodes: SvgPath[];
  constructor(params: TypeProps = {}) {
    super();
    this.className = 'FlSaveEditRegularSvg';
    this.attr.addObj({
      name: 'FlSaveEditRegularSvg',
      title: 'FlSaveEditRegularSvg'
    });
    this.attr.addObj({
      viewBox: '0 0 20 20',
    });
    this.resetSize(24, 24);
    this.childNodes = [];
    const path0 = new SvgPath({ attrObj: { fill: 'currentColor' }});
    path0.setData(
      'M5 3C3.89543 3 3 3.89543 3 5V15C3 16.1046 3.89543 17 5 17H8.00383C8.01113 16.8859 8.02893 16.7701 8.05813 16.6533L8.22145 16H6V11.5C6 11.2239 6.22386 11 6.5 11H11.9436L12.9436 10H6.5C5.67157 10 5 10.6716 5 11.5L5 16C4.44772 16 4 15.5523 4 15V5C4 4.44772 4.44772 4 5 4L6 4V6.5C6 7.32843 6.67157 8 7.5 8L11.5 8C12.3284 8 13 7.32843 13 6.5V4L13.3787 4C13.6439 4 13.8983 4.10536 14.0858 4.29289L15.7071 5.91421C15.8946 6.10175 16 6.3561 16 6.62132V8.00304C16.3367 7.98758 16.6757 8.0311 17 8.13362V6.62132C17 6.09089 16.7893 5.58218 16.4142 5.20711L14.7929 3.58579C14.4178 3.21071 13.9091 3 13.3787 3H5ZM7 6.5L7 4L12 4V6.5C12 6.77614 11.7761 7 11.5 7L7.5 7C7.22386 7 7 6.77614 7 6.5ZM14.8092 9.54776L9.97975 14.3772C9.69818 14.6588 9.49842 15.0116 9.40184 15.3979L9.02737 16.8958C8.86451 17.5472 9.45456 18.1372 10.106 17.9744L11.6039 17.5999C11.9902 17.5033 12.343 17.3036 12.6246 17.022L17.454 12.1926C18.1843 11.4622 18.1843 10.2781 17.454 9.54776C16.7237 8.81741 15.5395 8.81741 14.8092 9.54776Z',
    );
    this.addChild(path0);
    this.useParams(params);
  }
}
