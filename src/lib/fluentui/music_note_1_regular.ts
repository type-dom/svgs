import { ITypeConfig, SvgPath, TypeSvgSvg } from '@type-dom/framework';
export class FlMusicNote1RegularSvg extends TypeSvgSvg {
  className: 'FlMusicNote1RegularSvg';
  override childNodes: SvgPath[];
  constructor(params: ITypeConfig = {}) {
    super();
    this.className = 'FlMusicNote1RegularSvg';
    this.attr.addObj({
      name: 'FlMusicNote1RegularSvg',
      title: 'FlMusicNote1RegularSvg'
    });
    this.attr.addObj({
      viewBox: '0 0 20 20',
    });
    this.resetSize(24, 24);
    this.childNodes = [];
    const path0 = new SvgPath({ attrObj: { fill: 'currentColor' }});
    path0.setData(
      'M10.147 2.02211C9.99539 1.97545 9.83058 2.00354 9.70295 2.09781C9.57531 2.19208 9.5 2.34133 9.5 2.5V12.4048C8.91605 11.8444 8.12325 11.5 7.25 11.5C5.45507 11.5 4 12.9551 4 14.75C4 16.5449 5.45507 18 7.25 18C9.04493 18 10.5 16.5449 10.5 14.75C10.5 14.6897 10.4984 14.6297 10.4951 14.5702C10.4983 14.5473 10.5 14.5238 10.5 14.5V7.17698L16.353 8.97789C16.5046 9.02456 16.6694 8.99647 16.7971 8.9022C16.9247 8.80793 17 8.65868 17 8.5V5.97715C17 4.87964 16.2842 3.91047 15.2352 3.58771L10.147 2.02211ZM16 7.82302L10.5 6.13072V3.17698L14.9411 4.54348C15.5705 4.73714 16 5.31865 16 5.97715V7.82302ZM5 14.75C5 13.5074 6.00736 12.5 7.25 12.5C8.49264 12.5 9.5 13.5074 9.5 14.75C9.5 15.9926 8.49264 17 7.25 17C6.00736 17 5 15.9926 5 14.75Z',
    );
    this.addChild(path0);
    this.useParams(params);
  }
}
